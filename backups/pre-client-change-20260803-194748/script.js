    ...
    try {
        ...
        if (Array.isArray(data.projects)) {
            ...
            optionsData.splice(2, optionsData.length - 2,
                { category: 'investment', title: 'Выбрать проект', min_amount_key: 'min_investment', project_id: '', placeholder: true },
                { category: 'investment', title: 'Предложить свой', min_amount_key: 'min_investment', project_id: '-1', project_code: 'OWN', custom: true },
                ...projects.map(p => ({
                    category: 'investment', title: p.name, min_amount_key: 'min_investment',
                    project_id: String(p.id), project_code: p.code, description: p.description || '', media_url: p.media_url || '',
    ...
    .then(data => {
        ...
        if (data.requisites && dynamicRequisites) {
            ...
            resultStatusText.innerText = currentCategory === 'investment'
                ? 'Сохраните этот код: он идентифицирует вашу заявку, транзакцию и комментарий к платежу.'

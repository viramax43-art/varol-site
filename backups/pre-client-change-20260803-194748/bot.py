...
async def panel_projects_start(callback: types.CallbackQuery):
    ...
    if not projects:
        ...
        for p in projects:
            ...
            text += f"ID {p['id']}: <code>{p['code']}</code> — {html.escape(p['name'])} (минимум: {minimum})\n"
            kb.row(
...
async def project_url_finish(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    if data.get("editing"):
        db.update_project(data["project_id"], data["name"], data["code"], data["min_amount"], data["description"], media_url)
        action = "обновлён"
    else:
        db.add_project(data["name"], data["code"], data["min_amount"], data["description"], media_url)
        action = "добавлен"
    await state.clear()

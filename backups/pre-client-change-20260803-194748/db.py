...
def init_db():
    ...
    # Backfill legacy rows before enforcing uniqueness. P<base36 id> is stable,
    # short, and does not depend on the project's language or punctuation.
    ...
    for row in project_rows:
        ...
        if not is_valid or raw_code in used_codes:
            ...
            c.execute('UPDATE projects SET code = ? WHERE id = ?', (raw_code, row['id']))
        elif raw_code != row['code']:
            c.execute('UPDATE projects SET code = ? WHERE id = ?', (raw_code, row['id']))
        used_codes.add(raw_code)
    c.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_projects_code_unique ON projects(code)')
    c.execute('''
        CREATE TRIGGER IF NOT EXISTS projects_code_required_insert
        BEFORE INSERT ON projects
        WHEN NEW.code IS NULL OR length(NEW.code) < 2 OR length(NEW.code) > 12
             OR NEW.code GLOB '*[^A-Z0-9]*' OR NEW.code = 'OWN'
        BEGIN SELECT RAISE(ABORT, 'invalid project code'); END
    ''')
    c.execute('''
        CREATE TRIGGER IF NOT EXISTS projects_code_required_update
...
def get_all_projects():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, code, description, media_url, min_amount FROM projects ORDER BY id")
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]

def add_project(name: str, code: str, min_amount=None, description: str = "", media_url: str = ""):
    code = normalize_project_code(code)
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO projects (name, code, min_amount, description, media_url) VALUES (?, ?, ?, ?, ?)",
        (name, code, min_amount, description, media_url),
    )
    conn.commit()
...
def get_project(project_id: int):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, code, description, media_url, min_amount FROM projects WHERE id = ?", (project_id,))
    row = cursor.fetchone()
    conn.close()
...
def get_project_by_code(code: str):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id, name, code, description, media_url, min_amount FROM projects WHERE code = ?", (code,))
    row = cursor.fetchone()
    conn.close()
    return dict(row) if row else None

def update_project(project_id: int, name: str, code: str, min_amount=None, description: str = "", media_url: str = ""):
    code = normalize_project_code(code)
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE projects SET name = ?, code = ?, min_amount = ?, description = ?, media_url = ? WHERE id = ?",
        (name, code, min_amount, description, media_url, project_id),
    )
    conn.commit()

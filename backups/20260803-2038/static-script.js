    ...
    try {
        ...
        if (Array.isArray(data.projects)) {
            const projects = data.projects.length ? data.projects : [
                { id: 'global-expansion-fund', name: 'Global Expansion Fund' },
                { id: 'tech-infrastructure-development', name: 'Tech Infrastructure Development' }
            ];
            optionsData.splice(2, optionsData.length - 2,
                { category: 'investment', title: 'Выбрать проект', min_amount_key: 'min_investment', project_id: '', placeholder: true },
                ...projects.map(p => ({ category: 'investment', title: p.name, min_amount_key: 'min_investment', project_id: String(p.id) })),
                { category: 'investment', title: 'Предложить свой', min_amount_key: 'min_investment', project_id: '-1' }
            );
...
function renderCustomDropdown() {
    ...
    if (filtered.length > 0) {
        selectDropdownItem(filtered[0].title, filtered[0].min_amount_key, filtered[0].project_id || '');
        filtered.filter(opt => !opt.placeholder).forEach(opt => {
            ...
            item.innerHTML = `<div style="font-weight:700">${opt.title}</div>`;
            item.onmouseenter = () => item.style.background = "var(--accent-bg-glow)";
            item.onmouseleave = () => item.style.background = "transparent";
            item.onclick = () => selectDropdownItem(opt.title, opt.min_amount_key, opt.project_id || '');
...
function updateMinAmountDisplay() {
    ...
    if (amountInput && amountInput.dataset.minKey) {
        const minVal = projectLimits[amountInput.dataset.minKey] || 5;
...
function selectDropdownItem(title, minKey, projectId = '') {
    ...
    if (amountInput) amountInput.dataset.minKey = minKey;
...
function onCountryInput(e) {
    ...
    const matches = countryDictionary.filter(c => {
        return c.name.toLowerCase().includes(query) ||
               c.keywords.some(k => k.includes(query) || query.includes(k));
    });
    ...
    matches.slice(0, 6).forEach(m => {
        ...
        item.innerHTML = `<span>${m.flag} ${m.name}</span>`;

...
async def panel_projects_start(callback: types.CallbackQuery):
    ...
    if not projects:
        ...
        for p in projects:
            ...
            kb.row(InlineKeyboardButton(text=f"❌ Удалить {p['id']}", callback_data=f"del_proj_{p['id']}"))
...
async def add_proj_finish(message: types.Message, state: FSMContext):
    if not is_admin(message.chat.id):
        return
    db.add_project(message.text.strip())
    await state.clear()
    await message.answer(f"Проект «{message.text}» успешно добавлен!\nВернуться к проектам: /panel", parse_mode="HTML")

...
let projectLimits = { min_donation: 5, min_investment: 250 };
const exchangeRateCache = new Map();
let banksFromDB = []; // banks loaded from DB for "Предложить свой"

const optionsData = [
    { category: 'donation', title: 'Поддержка платформы (Донат)', min_amount_key: 'min_donation' },
...
async function loadConfig() {
    try {
        ...
        projectLimits.min_investment = parseFloat(data.min_investment) || 250;
        projectLimits.own_project_min = parseFloat(data.own_project_min) || 250;
        
        if (data.anon_title) {
            optionsData[1].title = data.anon_title;
        }
        
        // Load banks from DB (region ALL or any active)
        let allBanks = Object.values(reqData.requisites || {}).flat().filter(b => b.is_active);
        if (allBanks.length === 0) {
            allBanks = Object.values(reqData.requisites || {}).flat();
        }
        banksFromDB = allBanks;
        buildPMDropdown(allBanks);
        
        if (Array.isArray(data.projects)) {
            const projects = data.projects;
            const ownTitle = data.own_project_title || 'Предложить свой проект';
            const ownMin = parseFloat(data.own_project_min) || 250;
            const ownOption = data.own_project_enabled
                ? [{ category: 'investment', title: ownTitle, min_amount_key: 'own_project_min', min_amount: ownMin, project_id: '-1', project_code: 'OWN', custom: true }]
                : [];

            // Real investment projects (filter out 'PC', 'OWN', and 'Донат' to prevent duplicate or conflicting items)
            const investmentProjects = projects
                .filter(p => (!p.category || p.category === 'investment') && p.is_active && p.code !== 'PC' && p.code !== 'OWN' && !p.name.toLowerCase().includes('предложить свой'))
                .map(p => ({
                    category: 'investment',
                    title: p.name,
                    min_amount_key: 'min_investment',
            ...
            const donationList = [
                { category: 'donation', title: 'Поддержка платформы (Донат)', min_amount_key: 'min_donation' },
                { category: 'donation', title: data.anon_title || 'Анонимный взнос', min_amount_key: 'min_donation' }
            ];
...
// Build the bank selection dropdown from DB banks
function buildPMDropdown(banks) {
    ...
    banks.forEach(b => {
        ...
        // Determine value key: if has link_url, it's a link-pay bank; otherwise requisites
        const val = b.link_url ? `link_${b.id}` : `bank_${b.id}`;
        item.onclick = () => selectPMFromDB(b);
...
function drawMolecular() {
    if (document.hidden) {
        if (!isMobile) requestAnimationFrame(drawMolecular);
        return;
    }
...
document.addEventListener("DOMContentLoaded", () => {
    renderCustomDropdown();
    switchCategory('investment');  // Default to investment
...
function switchCategory(cat) {
    ...
    const tabDon = document.getElementById('tabDonation');
    const tabInv = document.getElementById('tabInvestment');
    if (tabDon) tabDon.classList.toggle('active', cat === 'donation');
    if (tabInv) tabInv.classList.toggle('active', cat === 'investment');

    const wrapper = document.getElementById('formWrapper');
    if (wrapper) { wrapper.classList.remove('form-wrapper-animate'); wrapper.style.opacity = '0.3'; }
    
    setTimeout(() => {
        const amountLabel = document.getElementById('amountLabelText');
        const commentLabel = document.getElementById('commentLabelText');
        const contactInput = document.getElementById('contactInput');
        const customNameGroup = document.getElementById('customProjectNameGroup');
        const customNameInput = document.getElementById('customProjectNameInput');
        ...
        if (cat === 'donation') {
            ...
            if (amountLabel) amountLabel.innerText = 'Сумма перевода €';
            if (commentLabel) commentLabel.innerHTML = 'Комментарий / Пожелание <span class="field-req-note">(необязательно)</span>';
            if (contactInput) { contactInput.required = false; }
            if (paymentMethodGroup) paymentMethodGroup.style.display = 'block';
            toggleAnonMode(true);
            if (submitBtn) submitBtn.style.display = 'flex';
            if (submitBtnText) submitBtnText.innerText = 'Подтвердить перевод';
        } else { // investment
            if (amountLabel) amountLabel.innerText = 'Сумма инвестиций €';
            if (commentLabel) commentLabel.innerHTML = 'Детали / Комментарий <span class="field-req-note">(необязательно)</span>';
            if (contactInput) { contactInput.required = false; }
            toggleAnonMode(false);
            if (submitBtnText) submitBtnText.innerText = 'Инвестировать в проект';
        }
        renderCustomDropdown();
        const amountInputEl = document.getElementById('amountInput');
        if (amountInputEl) amountInputEl.value = '';
        if (wrapper) { wrapper.style.opacity = '1'; wrapper.classList.add('form-wrapper-animate'); }
...
function updateMinAmountDisplay(forceReset = false) {
    const amountInput = document.getElementById("amountInput");
    
    if (amountInput && amountInput.dataset.minKey) {
        const optionVal = document.getElementById("optionSelectValue");
        const isPlaceholder = optionVal && (!optionVal.dataset.projectId || optionVal.dataset.projectId === "") && currentCategory === "investment";
        ...
        if (isPlaceholder) {
            ...
            amountInput.placeholder = `${minVal}.00`;
            if (forceReset || !amountInput.value || parseFloat(amountInput.value) < minVal) {
        ...
        if (amountLabel) {
            if (currentCategory === 'investment') {
                if (isPlaceholder) {
                    amountLabel.innerText = `Сумма инвестиций €`;
                } else {
                    amountLabel.innerText = `Сумма инвестиций € (порог от ${minVal} €)`;
                }
            } else {
                amountLabel.innerText = `Сумма перевода € (от ${minVal} €)`;
            }
        }
    }
    fetchExchangeRate();
...
function selectDropdownItem(title, minKey, projectId = '', option = null) {
    ...
    if (optionVal) {
        ...
        optionVal.dataset.projectCode = option && option.project_code ? option.project_code : '';
        optionVal.dataset.paymentLink = option && option.payment_link ? option.payment_link : '';
    }
    if (amountInput) {
    ...
    if (option && (option.custom || projectId === '-1')) {
        // "Предложить свой" — manual entry, show payment method picker
        if (customNameGroup) customNameGroup.style.display = 'block';
        if (commentLabel) commentLabel.innerHTML = 'Детали / описание проекта <span class="field-req-note">(необязательно)</span>';
        if (paymentMethodGroup) paymentMethodGroup.style.display = 'block';
        if (banksFromDB.length > 0) {
            selectPMFromDB(banksFromDB[0]);
        }
    } else if (option && option.category === 'donation') {
        // Donation — show payment method picker (user picks bank/link)
        if (customNameGroup) customNameGroup.style.display = 'none';
        if (customNameInput) customNameInput.value = '';
    ...
    } else if (option && option.project_id && option.project_id !== '' && option.project_id !== '-1' && !option.placeholder) {
        // Real investment project — redirect via Revolut link, no PM picker shown
        if (customNameGroup) customNameGroup.style.display = 'none';
        if (customNameInput) customNameInput.value = '';
        if (commentLabel) commentLabel.innerHTML = 'Комментарий <span class="field-req-note">(необязательно)</span>';
        if (paymentMethodGroup) paymentMethodGroup.style.display = 'none';
        document.getElementById("pmDropdownSelectedText").innerText = 'Revolut';
        document.getElementById("paymentMethodValue").value = 'revolut';
        // Show submit button for direct pay
        const submitBtn = document.getElementById("submitBtn");
        if (submitBtn) submitBtn.style.display = 'block';
    } else {
        // Placeholder or unknown — hide everything
        if (customNameGroup) customNameGroup.style.display = 'none';
        if (customNameInput) customNameInput.value = '';
        if (paymentMethodGroup) paymentMethodGroup.style.display = 'none';
        const submitBtn = document.getElementById("submitBtn");
    ...
    // forceReset=true при смене проекта
    updateMinAmountDisplay(true);
...
function updateSubmitButtonText() {
    ...
    const optionVal = document.getElementById("optionSelectValue");
    const isCustom = optionVal && optionVal.dataset.projectId === '-1';
    
    if (text) {
        text.innerText = isCustom ? 'Подтвердить' : 'Оплатить';
    }
}
...
function submitPayment(e) {
    ...
    const pmValue = document.getElementById("paymentMethodValue").value;
    if (!pmValue) {
        alert("Пожалуйста, выберите метод оплаты.");
        return;
    }

    if (pmValue !== 'revolut' && !pmValue.startsWith('link_') && (!fileInput || fileInput.files.length === 0)) {
        alert("Пожалуйста, загрузите скриншот чека об оплате.");
        return;
    }
    ...
    let fullCat = `${currentCategory === 'investment' ? 'Инвестор' : 'Донат'}: ${category}`;
    ...
    formData.append("category", fullCat);
    formData.append("feedback_requested", feedbackRequested ? "true" : "false");
    
    if (currentCategory === 'investment') {
        const selectedProject = document.getElementById("optionSelectValue");
        formData.append("project_id", selectedProject.dataset.projectId);
        formData.append("project_code", selectedProject.dataset.projectCode || "");
        
        if (selectedProject.dataset.projectId === '-1') {
            const customNameEl = document.getElementById("customProjectNameInput");
            const customName = customNameEl ? customNameEl.value.trim() : "";
            if (!customName) {
                alert("Пожалуйста, укажите название вашей инвестиции / проекта.");
                if (customNameEl) customNameEl.focus();
                btn.disabled = false;
                btn.innerHTML = originalText;
                return;
            }
            formData.append("custom_project_name", customName);
        }
    }
    
    const pmDropdownText = document.getElementById("pmDropdownSelectedText").innerText;
    formData.append("payment_method", pmDropdownText);
    formData.append("comment", comment || "");

    // If it's a direct link bank (link_ID) or revolut — redirect immediately, no API call
    if (pmValue === 'revolut' || pmValue.startsWith('link_')) {
        let redirectUrl = '';
        if (pmValue.startsWith('link_')) {
            // Find the bank from DB by ID
            const bankId = parseInt(pmValue.replace('link_', ''));
            const bank = banksFromDB.find(b => b.id === bankId);
            redirectUrl = bank ? bank.link_url : '';
        }
        if (!redirectUrl) {
            // Fallback: use project-specific link or default revolut
            const optionVal = document.getElementById("optionSelectValue");
            const customLink = (currentCategory === 'investment' && optionVal && optionVal.dataset.paymentLink) ? optionVal.dataset.paymentLink : '';
            redirectUrl = customLink || 'https://checkout.revolut.com/pay/c3ed9a90-fc91-420f-83f1-14b27310a744';
        }
        btn.disabled = false;
        btn.innerHTML = originalText;
    ...
    .then(data => {
        ...
        if (data.requisites && dynamicRequisites) {
            ...
            resultStatusText.innerText = currentCategory === 'investment'
                ? 'Ваша заявка и чек успешно отправлены.'
                : 'Заявка успешно зафиксирована. Чек прикреплен.';
        }
    })
    .catch(err => {
...
function togglePMDropdown() {
    const pmDropdown = document.getElementById("paymentMethodDropdown");
    if (pmDropdown) pmDropdown.classList.toggle("open");
}

// Select payment method from DB bank object
function selectPMFromDB(bank) {
    document.getElementById("pmDropdownSelectedText").innerText = bank.name;
    document.getElementById("paymentMethodValue").value = bank.link_url ? `link_${bank.id}` : `bank_${bank.id}`;
    document.getElementById("paymentMethodDropdown").classList.remove("open");
    
    const reqBlock = document.getElementById("requisitesBlock");
    const reqText = document.getElementById("requisitesText");
    const receiptUploadGroup = document.getElementById("receiptUploadGroup");
    
    if (bank.link_url) {
        // Direct payment link — show gold button, no receipt needed
        if (reqText) reqText.innerHTML = `<div style="line-height:1.6; text-align: left;">
<div style="font-weight:800; font-size:16px; margin-bottom:12px; color:#fff; font-family:'Plus Jakarta Sans',sans-serif;">${bank.name}</div>
<a href="${bank.link_url}" target="_blank" rel="noopener" class="pay-action-btn">
  <span class="material-symbols-rounded" style="font-size:20px; margin-right:8px; vertical-align:middle;">open_in_new</span>
  <span>Перейти к оплате</span>
</a>
<div class="pay-link-url">${bank.link_url}</div>
</div>`;
        if (reqBlock) reqBlock.style.display = "block";
        if (receiptUploadGroup) receiptUploadGroup.style.display = "none";
    } else {
        // Bank requisites — show requisites + upload
        const value = (bank.value || '').replace(/\n/g, '<br>');
        if (reqText) reqText.innerHTML = `<div style="font-family:'JetBrains Mono',monospace; font-size:13.5px; line-height:1.8; text-align: left;">
<div style="font-weight:800; font-size:16px; margin-bottom:10px; color:#fff; font-family:'Plus Jakarta Sans',sans-serif;">${bank.name}</div>
<div style="background: rgba(0,0,0,0.4); padding: 14px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.08); color: #e2e8f0; word-break: break-all;">${value}</div>
</div>`;
        if (reqBlock) reqBlock.style.display = "block";
...
// Legacy selectPM kept for compatibility (uses first matching bank from DB or no-op)
function selectPM(title, value) {
    // Try to find matching bank from DB by name
    const match = banksFromDB.find(b => b.name === title);
    if (match) { selectPMFromDB(match); return; }

    // Revolut default (for project selections with payment_link)
    document.getElementById("pmDropdownSelectedText").innerText = title;
    document.getElementById("paymentMethodValue").value = value;
    document.getElementById("paymentMethodDropdown").classList.remove("open");
...
function updateFileName(input) {
    ...
    if (input.files && input.files.length > 0) {
        display.innerText = "Файл: " + input.files[0].name;
        display.style.color = "#10B981";
    } else {
        display.innerText = "Прикрепить чек... (обязательно)";
...
function toggleAnonMode(isAnon) {
    ...
    // Hide/show contact fields
    if (contactGroup) {
        contactGroup.style.display = isAnon ? 'none' : 'block';
    }
    if (contactInput) {
        contactInput.required = !isAnon;
    ...
    // Clean class-based sync for smooth CSS animations
    const toggleWrap = document.querySelector('.anon-toggle');
    if (toggleWrap) {
        if (isAnon) {
...
function openSiteQrModal() {
    ...
    const urlDisplay = document.getElementById("siteQrUrlDisplay");
    if (!modal || !box) return;
...
function downloadSiteQr() {
    ...
    if (dataUrl) {
        const link = document.createElement("a");
        link.download = "varol-service-qr.png";
        link.href = dataUrl;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
    } else {
        alert("Не удалось сгенерировать изображение QR-кода");
    }
}

function copySiteUrl(event) {
    const currentUrl = window.location.origin + window.location.pathname;
    navigator.clipboard.writeText(currentUrl).then(() => {
        const btn = event.currentTarget;
        const originalText = btn.innerHTML;
        btn.innerHTML = "<span class='material-symbols-rounded' style='font-size: 16px;'>check</span><span>Скопировано!</span>";
